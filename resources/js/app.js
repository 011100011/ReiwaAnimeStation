...
let minusCart = document.querySelectorAll('.minus-btn')
let minusCartCounter = document.querySelector('#cartCounter')

function minusUpdateCart(order){
  axios.post('/update-minus-cart', order).then(res => {
    minusCartCounter.innerText = res.data.totalQty
    new Noty({
      type: 'success',
      timeout: 1000,
      text: '1 item remove to cart',
      progressBar: false
    }).show();
  }).catch(err => {
    new Noty({
      type: 'error',
      timeout: 1000,
      text: "There's no item in cart",
      progressBar: false
    }).show();
  })
}

minusCart.forEach((btn) => {
  btn.addEventListener('click', (e) => {
    let order = JSON.parse(btn.dataset.order)
    minusUpdateCart(order)
  })
})
...
const dropdown2 = document.getElementById("options-menu2");

// navbar dropdown open with click

if (dropdown2) {
  dropdown2.addEventListener("click", () => {
    document.querySelector(".dropdown-menu2").classList.toggle("show");
  });

  window.onclick = function (event) {
    if (!event.target.matches("#options-menu2")) {
